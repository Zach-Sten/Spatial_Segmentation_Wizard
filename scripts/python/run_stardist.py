...
def _install_worker_patch(src_model_dir: Path):
    """Inject a sitecustomize.py so every dask worker process loads StarDist2D
    from a local directory instead of calling keras.get_file (which races when
    multiple workers extract the same zip simultaneously).

    sitecustomize.py is run by Python at interpreter startup before any user
    code, making it the only way to patch code in spawned worker processes
    without modifying installed packages.  Workers inherit PYTHONPATH (env var),
    unlike in-memory patches which don't survive spawn.
    """
    import tempfile
    import textwrap

    basedir = str(src_model_dir.parent.resolve())  # e.g. .../models/StarDist2D/

    sitecust = textwrap.dedent(f"""\
        # Auto-generated by run_stardist.py.
        # Patches StarDist2D.from_pretrained to load from a pre-staged local
        # directory, bypassing keras.get_file and its concurrent-extraction race.
        try:
            from stardist.models import StarDist2D as _SD
            from pathlib import Path as _P
            _basedir = _P(r"{basedir}")
            _orig = _SD.from_pretrained.__func__

            @classmethod
            def _local_from_pretrained(cls, name_or_alias):
                p = _basedir / name_or_alias
                if p.is_dir():
                    return cls(None, name=str(p.name), basedir=str(p.parent))
                return _orig(cls, name_or_alias)

            _SD.from_pretrained = _local_from_pretrained
        except Exception as _e:
            import sys
            print(f"[WARN] stardist worker patch failed: {{_e}}", file=sys.stderr)
    """)

    tmp_dir = Path(tempfile.mkdtemp(prefix="stardist_patch_"))
    (tmp_dir / "sitecustomize.py").write_text(sitecust)

    existing = os.environ.get("PYTHONPATH", "")
    os.environ["PYTHONPATH"] = str(tmp_dir) + (":" + existing if existing else "")
    print(f"[INFO] Worker patch installed: PYTHONPATH prepended with {tmp_dir}")
    print(f"[INFO] Workers will load StarDist2D directly from: {basedir}")
...
def main():
    ...
    if src_base is not None:
        ...
        if candidate.is_dir():
            ...
            _install_worker_patch(candidate)
        else:
