...
def compute_segger_metrics(method_data: dict, qc_dir: Path, base_dir: Path,
    ...
    for each method that has pred_cell_type annotations.
    ...
    for method, (adata, output_dir) in method_data.items():
        ...
        # Alias pred_cell_type to celltype_major (used by segger functions)
        adata = adata.copy()
        adata.obs["celltype_major"] = adata.obs["pred_cell_type"].astype(str)
...
def main():
    ...
    for method, (adata, output_dir) in method_data.items():
        ...
        if annot_csv.exists():
            ...
            if method == "xenium":
                valid_frac = adata.obs.get("pred_cell_type", pd.Series(dtype=str)).notna().mean()
