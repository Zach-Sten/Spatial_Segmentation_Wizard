"""
segger_functions — Segmentation comparison metrics and plotting.
...
Functions for computing:
  ...
  - Clustering scores (CH, Silhouette)
...
def plot_contamination_heatmaps(contam_dict, figsize=(8, 6), cmap="Reds", vmin=0, vmax=None):
    num_methods = len(contam_dict)
    fig, axes = plt.subplots(1, num_methods, figsize=(figsize[0] * num_methods, figsize[1]), squeeze=False)

    for i, (method, df) in enumerate(contam_dict.items()):
        ax = axes[0, i]
        sns.heatmap(df, annot=False, cmap=cmap, ax=ax, cbar=True, vmin=vmin, vmax=vmax,
                    square=True, linewidths=0.5, linecolor='gray', fmt=".2f")
